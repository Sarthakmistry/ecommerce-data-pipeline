...
dbt staging models
       ...
       ▼
┌─────────────────────┐
│  staging_checkpoint  │◄── ge_validate_staging (Airflow PythonOperator)
│  4 suites, ~25      │
│  expectations        │──── failures ──► QUARANTINE table
└─────────────────────┘
       │ pass
       ▼
dbt vault → snapshots → marts
       │
       ▼
┌─────────────────────┐
│   marts_checkpoint   │◄── ge_validate_marts (Airflow PythonOperator)
│  3 suites, ~30      │
│  expectations        │──── failures ──► QUARANTINE table
└─────────────────────┘
       │ pass
